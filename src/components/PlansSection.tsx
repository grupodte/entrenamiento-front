...
const featuredPlan = {
  ...
  badge: 'Ahorro USD 50',
  price: 'USD 300',
  note: 'Pagás menos que contratando ambos por separado.',
  duration: '2 meses',
  features: [
    ...
    'Guía de ejercicios y hábitos para sostenerlo',
  ],
}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 items-stretch">
            ...
            <div className="plan-spotlight order-1 md:order-2 relative rounded-[20px] sm:rounded-[24px] p-[1px] md:col-span-1">
              ...
              <div className="relative flex h-full flex-col overflow-hidden rounded-[20px] sm:rounded-[24px] bg-[#9580A6] p-5 sm:p-6 md:p-8 shadow-[0_18px_48px_rgba(149,128,166,0.24)]">
                ...
                <p className="relative z-10 text-white/82 text-[12px] sm:text-[13px] leading-[1.4] mb-2 m-0">
                  {featuredPlan.note}
                </p>
                <p className="relative z-10 text-white/70 text-[13px] mb-7 m-0">{featuredPlan.duration}</p>

                <ul className="relative z-10 flex flex-col gap-2.5 mb-8 m-0 p-0 list-none flex-1">
                  {featuredPlan.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2.5 text-[13px] sm:text-[14px] text-white leading-[1.35] tracking-[-0.015em]">
